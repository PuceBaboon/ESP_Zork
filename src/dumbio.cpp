/*
 *    $Id: dumbio.cpp,v 1.3 2017/04/10 14:49:34 gaijin Exp $
 *
 * $Id: dumbio.cpp,v 1.3 2017/04/10 14:49:34 gaijin Exp $   
 * --------------------------------------------------------------------
 ...
 * --------------------------------------------------------------------
 * 
 * File name: $Id: dumbio.cpp,v 1.3 2017/04/10 14:49:34 gaijin Exp $  
 *   
 * Description:    
 *    
 * Modification history:      
 * $Log: dumbio.cpp,v $
 * Revision 1.3  2017/04/10 14:49:34  gaijin
 *    Commented out feof() check.
 *    Added screen-related defines attop of file.
 *    Replaced input and output code with "Serial.***" calls.
 *
 *    <<WORKING, BUT NO DIFFERENCE OR IMPROVEMENT OVER ORIGINAL VERSION>>
 *
 * Revision 1.2  2017/04/10 00:24:09  gaijin
 *    NO CODE CHANGES.
 *
 *    Run through indent to get rid of annoying extra space characters.
 *
 * Revision 1.1  2017/04/10 00:23:12  gaijin
 *    Import of dumbio.c from JZip original source.
 *
 *    Updated "inc()" to "in_c()", "uninc()" to "unin_c()" and "outc()" to
 *    "out_c()" to prevent me thinking "increment" every time I see them.
 *
 *    Updated to use Stream-I/O instead of fprintf and faminly.
 *
...
//         fprintf( stderr, "\nEOT\n" );
//         exit( 0 );
//            esp_osfatal("EOT  -  dumbio.cpp");
//        } else {
//         fprintf( stderr, "zip: dumbio.c: inc: error in getchar\n" );
//         exit( 1 );
            esp_osfatal("EOF - getchar error  -  dumbio.c");
//        }
    }
    return c;
}

static int unin_c(int c) {
//    return ungetc(c, stdin);
     return 0;
}

static int out_c(int c) {
...
void initialize_screen(void) {
    ...
    dumb_prompt = (char *) malloc(screen_cols);
    dumb_status_window = (char *) malloc(screen_rows * screen_cols);
    dumb_old_status_window = (char *) malloc(screen_rows * screen_cols);
...
void create_status_window(void) {
    ...
    if (status_size > dumb_status_size) {
        ...
        memset(dumb_row(dumb_status_size), ' ',
               (status_size - dumb_status_size) * screen_cols);
    }
...
static void prompt_flush(void) {
    ...
    display_status_size =
        (dumb_status_max == 0) ? status_size : dumb_status_max;
    dumb_status_max = 0;
    ...
    for (r = start; r <= end; r++) {
        if ((status_compression == COMPRESSION_CHANGED)
            && (r < last_display_status_size) && (!dumb_row_changed(r)))
            continue;
        dumb_row_copy(r);
